...
import static com.hospital.config.HibernateUtils.con;
...
public class AppointmentService {
    ...
    public void haftalikRandevuTableList(TreeMap<String, LinkedList<LocalTime>> treeMap) throws IOException, InterruptedException, SQLException {
        ...
        String select = HospitalService.scan.nextLine().toLowerCase();
        ...
        while (devamMi) {
            ...
            gun = HospitalService.scan.next();
            HospitalService.scan.nextLine();
            System.out.println("Lütfen randevu almak istediğiniz saati giriniz:\n\t=> DIKKAT! Saati \"hh:mm\" formatında giriniz!");
            try {
                saat = LocalTime.parse(HospitalService.scan.nextLine(), DateTimeFormatter.ofPattern("HH:mm"));
            } catch (Exception e) {
        ...
        System.out.println("----------------------------------------------------------------------------");
        HospitalService.slowPrint("\033[33mANAMENU'YE YONLENDIRILIYORSUNUZ...\033[0m\n", 20);
        HospitalService.hospitalService.start();
    }
